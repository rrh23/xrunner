...
public class Spawner : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (isPlaying)
        {
            ...
            obstacleSpawnTime = Mathf.Max(obstacleSpawnTime - Time.deltaTime * 0.01f, 0.5f); //min. 0.5f
    ...
    private bool IsSpawnAreaClear(Vector3 position, float radius = 1f)
{
    ...
    private void Spawn()
    {
        ...
        //overlap checker
        Vector3 spawnPos = new Vector3(transform.position.x, Random.Range(lowestpos, highestpos));
        if (IsSpawnAreaClear(spawnPos, 1f))
        {
            ...
            else
            {
                ...
                else if(obstacleID >= 2 || obstacleID <= 3) //floating structures
                {
                    obstacleToSpawn = objectPools.SpawnRandomFromPools(new Vector3(transform.position.x, Random.Range(lowestpos, highestpos)), Quaternion.identity);
                    obstacleRB = obstacleToSpawn.GetComponent<Rigidbody2D>();
                    obstacleRB.velocity = Vector2.left * obstacleSpeed;
                ...
                else //big structures either stuck in ground or ceiling
                {
                    ...
                    {
                        obstacleToSpawn = objectPools.SpawnRandomFromPools(new Vector3(transform.position.x, Random.Range(-3, -1.9f)), Quaternion.identity);
                        obstacleRB = obstacleToSpawn.GetComponent<Rigidbody2D>();
                        obstacleRB.velocity = Vector2.left * obstacleSpeed;
                    }
                    else
                    {
                        obstacleToSpawn = objectPools.SpawnRandomFromPools(new Vector3(transform.position.x, Random.Range(2, 4)), Quaternion.Euler(0, 0, 180));
                        obstacleRB = obstacleToSpawn.GetComponent<Rigidbody2D>();
                        obstacleRB.velocity = Vector2.left * obstacleSpeed;
                    }
                }
        ...
        {
            Spawn();
            //god i hope recursion doesn't break this program
        }
    }
}
